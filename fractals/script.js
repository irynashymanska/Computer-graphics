...
const sin = canvas.getContext('2d');
...
function setColoredTheme(noDraw) {
    for (let n = 0; n < MAX_ITERATIONS; n++) {
        colors[n] = hslToRgb(n / MAX_ITERATIONS / 1, 10, n / MAX_ITERATIONS);
    }
    theme = 'color';
    if (!noDraw) draw();
}
...
function draw() {
    ...
    let imageData = sin.createImageData(width, height);
    ...
    sin.putImageData(imageData, 0, 0);
}
